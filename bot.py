...
import os
token = '8157630067:AAFIxMzAzgtwoJUGHYvF5OyBnKXVdqvvbYA'
bot=telebot.TeleBot(token,parse_mode="HTML")
subscriber = '6320857317'
@bot.message_handler(commands=["start"])
def start(message):
	if not str(message.chat.id) == subscriber:
		bot.reply_to(message, "You cannot use the bot to contact developers to purchase a bot subscription @khainezay")
		return
	bot.reply_to(message,"Send the file now")
@bot.message_handler(content_types=["document"])
def main(message):
	if not str(message.chat.id) == subscriber:
		bot.reply_to(message, "You cannot use the bot to contact developers to purchase a bot subscription @TryGuyFlashHand")
		return
	dd = 0
	live = 0
	ch = 0
...
𝒃𝒚 ➜ @TryGuyFlashHand ''', reply_markup=mes)
				msg = f'''◆ 𝑪𝑨𝑹𝑫  ➜ {cc} 
◆ 𝑺𝑻𝑨𝑻𝑼𝑺 ➜ 𝘼𝙋𝙋𝙍𝙊𝙑𝙀𝘿 🔥
